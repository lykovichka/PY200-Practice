...
class LinkedList:
    ...
    def init_linked_list(self, data: Iterable):
        ...
        self.list_nodes = [Node(value) for value in
                           data]  # TODO обернуть все значения в класс Node и поместить их в python список

        for i in range(len(self.list_nodes) - 1):
            right = self.list_nodes[i + 1]

            left = self.list_nodes[i]

            self.linked_nodes(left, right)
